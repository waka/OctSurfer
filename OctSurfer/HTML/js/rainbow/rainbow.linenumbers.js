...
//			,iChars = iLines===0?1:(Math.log(iLines)/2.303<<0)+1 // 2.302585092994046 safely rounded to 2.303
        ...
        // row hightlight canvas
            ,iHightlight = block.getAttribute('data-highlight')
            ,mCnvRow = document.createElement('canvas')
            ,oCtxRow = mCnvRow.getContext('2d')
            ;
        /*var N="\n";console.log(
        ...
        // row hightlight canvas
        mCnvRow.setAttribute('width',1);
        mCnvRow.setAttribute('height',iLineHeight);
        oCtxRow.fillStyle = oParentStyle.borderColor;
        oCtxRow.fillRect(0,0,1,iLineHeight);
        ...
        if (bPre) {
            ...
            if (drawLines) {
                sBackground = 'url('+mCnvLines.toDataURL()+') 0 '+iTop+'px,'+sBackground;
            }
            if (highLightRow && iHightlight != null) {
                sBackground = 'url('+mCnvRow.toDataURL()+') 0 ' + (iTop + iLineHeight * +iHightlight) +'px repeat-x,' + sBackground;
            }
            if (drawNumbers) {
